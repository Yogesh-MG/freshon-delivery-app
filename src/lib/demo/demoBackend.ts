...
const freshState = (): DemoState => ({
  ...
  trips: seedTrips(),
  activeTripId: null,
  earnings: { earnings: 480, goal: 1500, deliveries: 4, distance: 21.6, rating: 4.8 },
});
  ...
  if (tripMatch && method === "POST") {
    ...
    switch (action) {
      ...
      case "reoptimize": {
        ...
        trip.total_distance_km = Math.round((trip.total_distance_km * 0.94) * 10) / 10;
    ...
    if (action === "deliver") {
      ...
      if (drops.every((s) => s.is_completed)) {
        ...
        state.earnings = {
          ...
          distance: Math.round((state.earnings.distance + trip.total_distance_km) * 10) / 10,
        };
      }
      return ok({}) as ApiResponse<T>;
    }
