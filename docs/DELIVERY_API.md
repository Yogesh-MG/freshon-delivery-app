...
| POST | `/assignments/{id}/deliver/` | `{ stop_id, type, otp_code, latitude, longitude }` | — |
...
The app sends more than this table lists: `photo_captured`, `proof_url`,
`exception_reason` and `accuracy_m` all cross the wire today and are all dropped
server-side. Each is the client half of a fix — see **§ 7a**, and
[SERVER_CHANGES.md](./SERVER_CHANGES.md) for the detail.

It sends **no** COD fields. The app does not collect cash.
...
which the endpoint reads yet. See **§ 7a**, S2. The photo itself is downscaled to a
...
`src/lib/cashDropService.ts` exists but **no UI references it**. Dead code, and now
deliberately so: the rider app does not collect cash on delivery, so there is no balance
for a rider to drop at the hub. The service is left in place for whenever COD returns.

---

## 7a. Required server changes

Moved. The full specification — every field the app sends that no endpoint reads yet,
with validation rules, edge cases and acceptance tests — lives in
**[SERVER_CHANGES.md](./SERVER_CHANGES.md)**. It is written to be handed to whoever works
on the backend, and it is the copy that gets maintained; this file stays a description of
...
| | Endpoint | Change |
| --- | --- | --- |
| S1 | `deliver/` | return a machine-readable `error_code` |
| S2 | `/proof/` | accept provenance (`stop_id`, `captured_at`, coordinates) and be idempotent on `capture_id` |
| S3 | `deliver/` | record and flag the `OTP_UNAVAILABLE` exception |
| S4 | `resend-otp/` | rate-limit, and report the limit in the response |
| S5 | `deliver/` | accept `photo_captured` so both proofs can be verified |
| S6 | `/api/auth/send-otp/` | rate-limit the rider login code |
| S7 | `deliver/` | be idempotent, so a lost response doesn't strand a rider |

**Cash on delivery is not on this list.** The rider app does not collect cash and sends no
COD fields; see SERVER_CHANGES.md § "On cash on delivery".
